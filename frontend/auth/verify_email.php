...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT id, email_verified FROM users WHERE verification_code=?");
...
// الرمز صحيح, قم بتحديث حالة التحقق
        $stmt = $conn->prepare("UPDATE users SET email_verified=1 WHERE verification_code=?");
        $stmt->bind_param("s", $entered_code);
        $stmt->execute();

        $_SESSION['email_verified'] = true;  // تحديث قيمة السيشن
// ربما تريد إعادة التوجيه هنا أو عرض رسالة نجاح

        header("Location: ../dashboard");
        exit();
    }else{
        $_SESSION['message'] = "Verification code isn't correct";
    }
}
?>
...
<!--                        <span class="input-group-text" id="inputGroupPrepend">@</span>-->
                  ...
                  <div class="card-footer">
                      <p><a href="resend_activation_code.php">Resend verification code</a></p> Or<p> <a href="logout.php">Logout</a> to start with another account</p>
                  </div>
              </div>
